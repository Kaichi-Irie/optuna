...
def vectorized_f19(
    x_flat: np.ndarray, R: np.ndarray, num_opt: int, dimension: int
) -> float:
    """
    f19関数のベクトル化バージョン。
    """
    ...
    torch.randn(num_opt, 300, 300) @ torch.randn(300, 300)  # some operation
...
#     # 1. 目的関数の値を一括で計算
#     start = time.time()
#     f_values = vectorized_f19(xs_batch.ravel(), R_matrix, N, D)
...
#     print("チェック計算時間:", time.time() - start)

#     for i in range(N):
#         if not (
#             np.isclose(f_values_check[i], f_values[i])
#             and np.allclose(grads_check[i], grads[i])
#         ):
#             print(f"Mismatch found at index {i}")
#             print(f"f19: {f_values_check[i]} vs {f_values[i]}")
